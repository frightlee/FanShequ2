...
class RepairActivity : AppCompatActivity() {
    ...
    @Event(R.id.img_back, R.id.btn_confirm, R.id.img_Add, R.id.rb_human_door, R.id.rb_unit_door, R.id.rb_park_door, R.id.rb_else_door, R.id.tv_cellName)
    private fun onCLicks(v: View) {
        when (v.id) {
            R.id.img_back -> finish()
            R.id.tv_cellName -> {
                pop = SpinerPopWindow(this, cellsList, AdapterView.OnItemClickListener { _, _, position, _ ->
